"""Phase 7.7 - Analyze transfer-learning training history."""
...
LOSS_PLOT_PATH = METADATA_DIR / "transfer_learning_loss_curve.png"
ACCURACY_PLOT_PATH = METADATA_DIR / "transfer_learning_accuracy_curve.png"
ANALYSIS_PATH = METADATA_DIR / "transfer_learning_analysis.json"
REQUIRED_SERIES = (
    "epochs",
    "train_loss",
    "train_accuracy",
...
def calculate_statistics(history):
    ...
    accuracy_gap = train_accuracy[best_index] - validation_accuracy[best_index]
    ...
    return {
        ...
        "best_validation_epoch": epochs[best_index],
        "initial_validation_loss": rounded(validation_loss[0]),
        "final_validation_loss": rounded(validation_loss[-1]),
        "training_accuracy_at_best_validation_epoch": rounded(train_accuracy[best_index]),
        "validation_loss_at_best_validation_epoch": rounded(validation_loss[best_index]),
        "training_validation_accuracy_gap_at_best_epoch": rounded(accuracy_gap),
    }
...
def create_plots(history):
    ...
    axis.set_title("Transfer Learning Training and Validation Loss")
    ...
    axis.set_title("Transfer Learning Training and Validation Accuracy")
    axis.set_xlabel("Epoch")
...
def factual_observations(history, statistics):
    ...
    observations = [
        (
            ...
            f"at epoch {epochs[0]} to {statistics['final_training_accuracy']:.6f} "
            f"at epoch {epochs[-1]}."
        ),
        (
            f"Validation accuracy was highest at epoch "
            f"{statistics['best_validation_epoch']} with a recorded value of "
            f"{statistics['best_validation_accuracy']:.6f}."
        ),
        (
            f"Validation loss at the highest-validation-accuracy epoch was "
            f"{statistics['validation_loss_at_best_validation_epoch']:.6f}."
        ),
        (
            f"The training-minus-validation accuracy difference at epoch "
            f"{statistics['best_validation_epoch']} was "
            f"{statistics['training_validation_accuracy_gap_at_best_epoch']:.6f}."
        ),
        (
            f"Validation loss changed from {statistics['initial_validation_loss']:.6f} "
            f"at epoch {epochs[0]} to {statistics['final_validation_loss']:.6f} "
            f"at epoch {epochs[-1]}."
        ),
    ]
    return observations
...
def print_summary(history, statistics, observations):
    print("\n" + "=" * 76)
    print("SMARTDOCS AI - PHASE 7.7 TRANSFER LEARNING TRAINING ANALYSIS")
    print("=" * 76)
    ...
    print(f"Best validation epoch                : {statistics['best_validation_epoch']}")
    ...
    print(
        ...
        f"{statistics['training_accuracy_at_best_validation_epoch']:.6f}"
    )
    print(
    ...
    try:
        ...
        report = {
            "training_summary": {
                ...
                "architecture": history.get("architecture"),
                "pretrained_weights": history.get("pretrained_weights"),
                "frozen_feature_status": history.get("frozen_feature_status"),
            },
            "calculated_statistics": statistics,
            "best_epoch": statistics["best_validation_epoch"],
            "best_validation_accuracy": statistics["best_validation_accuracy"],
            "training_validation_accuracy_gap_at_best_epoch": statistics[
                "training_validation_accuracy_gap_at_best_epoch"
            ],
            "metric_series": {
